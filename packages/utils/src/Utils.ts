...
import {encodeUserOp, NotPromise, packUserOp, UserOperation} from "./ERC4337Utils";
...
export async function packUserOpDapp(
	detailsForUserOp: TransactionDetailsForUserOp, provider: JsonRpcProvider, accounAddress: string, // account contract address
	entryPointAddress: string
): Promise<Partial<UserOperation>> {
	const {data, target} = detailsForUserOp
	const value = parseNumber(detailsForUserOp.value) ?? BigNumber.from(0);
	const accountContract = SimpleAccount__factory.connect(accounAddress, provider)
